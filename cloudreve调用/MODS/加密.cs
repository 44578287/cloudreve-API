...
namespace cloudreve.MODS
{
    internal class Encrypt
    {
        ...
        public static bool InitializationUser(string CookiePath, string UserName, string UserCookie, string UUID,int ExpirationTime = 2592000, bool Forced = false)
        {
            bool first = false;
            ...
            if (new DateTimeOffset(DateTime.UtcNow).ToUnixTimeSeconds() - long.Parse(KeyCookieList.Max()!) > ExpirationTime || Forced == true)//判断Cookie是否超过一个月以及是否强制生成
            {
                string? KeyCookie = DES.TextEncrypt(UserCookie, UUID);//加密用户Cookie
                File.WriteAllText(UserCookiePath + "/" + new DateTimeOffset(DateTime.UtcNow).ToUnixTimeSeconds().ToString() + ".cookie", KeyCookie);//以时间戳保存加密用户Cookie
                first = true;
